...
public class RailsAndRoadsOfTheRim : Mod
{
    public static RailsAndRoadsOfTheRimSettings settings;

    public static readonly List<TerrainDef> builtRoadTerrains = [];
    private static string currentVersion;

    public RailsAndRoadsOfTheRim(ModContentPack content) : base(content)
    {
        settings = GetSettings<RailsAndRoadsOfTheRimSettings>();
        ...
        float WinterOffset, out float BiomeModifier, out float HillModifier, out float WinterModifier)
    {
        ...
        //RailsAndRoadsOfTheRim.DebugLog("calculateRoadModifier: BiomeCoef=" +BiomeCoef+ ", BiomeMovementDifficulty="+ BiomeMovementDifficulty+ ", HillModifier"+ HillModifier+ ", HillinessOffset="+ HillinessOffset+ ", WinterModifier="+ WinterModifier+ ", WinterOffset="+ WinterOffset);
    ...
    */
    public static bool DoSomeWork(Caravan caravan, RoadConstructionSite site)
    {
        ...
        float ratio_final = 1;
        //RailsAndRoadsOfTheRim.DebugLog("[RotR] DEBUG ========== doSomeWork() ==========");
        //RailsAndRoadsOfTheRim.DebugLog("[RotR] DEBUG ISR2G set to "+useISR2G);
        ...
        //RailsAndRoadsOfTheRim.DebugLog("[RotR] ISR2G DEBUG ratio final = " + ratio_final);
        ...
        foreach (var aThing in CaravanInventoryUtility.AllInventoryItems(caravan))
        {
            foreach (var resourceName in DefModExtension_RotR_RoadDef.allResources)
            {
                if (!DefModExtension_RotR_RoadDef.GetInSituModifier(resourceName, useISR2G))
                {
                    ...
                    {
                        continue;
                        //RailsAndRoadsOfTheRim.DebugLog("[RotR] ISR2G consumption DEBUG =" + resourceName + " Qty consumed = " + amountUsed);
                    }
                ...
                else
                {
                    ...
                    //RailsAndRoadsOfTheRim.DebugLog("[RotR] ISR2G consumption DEBUG =" + resourceName + " Qty freely awarded = " + needed[resourceName]);
    ...
    public override void DoSettingsWindowContents(Rect rect)
    {
        ...
        {
            settings.BaseEffort = RailsAndRoadsOfTheRimSettings.MaxBaseEffort;
        }
        else
        {
            listing_Standard.Gap();
            ...
            settings.BaseEffort = (int)listing_Standard.Slider(settings.BaseEffort,
                RailsAndRoadsOfTheRimSettings.MinBaseEffort, RailsAndRoadsOfTheRimSettings.MaxBaseEffort);
        }
