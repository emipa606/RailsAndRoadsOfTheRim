...
namespace RailsAndRoadsOfTheRim.HarmonyPatches;
...
public class HarmonyPatches
{
    ...
    {
        new Harmony("Loconeko.Rimworld.RailsAndRoadsOfTheRim").PatchAll(Assembly.GetExecutingAssembly());
        ...
        */
        ...
        {
            //RailsAndRoadsOfTheRim.DebugLog("initialising roadDef " + thisDef);
                    ...
                    ) // Only get RoadDefGenStep_Place
            {
                ...
                if (!RailsAndRoadsOfTheRim.builtRoadTerrains.Contains(t))
                {
                    RailsAndRoadsOfTheRim.builtRoadTerrains.Add(t);
                }
            }
        }
    }
 ...
 * The actual rates are stored in static method RailsAndRoadsOfTheRim.calculateRoadModifier
 */
...
public static class Patch_WorldLayer_Paths_AddPathEndpoint
{
    ...
    {
        RailsAndRoadsOfTheRim.DebugLog("TRANSPILING");
        int index = -1;
        var codes = new List<CodeInstruction>(instructions);
        for (int i = 0; i < codes.Count; i++)
        {
            //RailsAndRoadsOfTheRim.DebugLog("Transpiler operand =" + codes[i].operand.ToStringSafe());
        ...
        if (index != -1)
        {
            ...
            RailsAndRoadsOfTheRim.DebugLog("Transpiler found 0.5 in AddPathEndPoint: " + codes[index].ToString());
        }
